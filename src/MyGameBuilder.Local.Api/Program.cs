...
using MyGameBuilder.Local.Api.Diagnostics;
...
catch (Exception exception) when (ShouldReportStartupFailure(exception))
{
    Environment.ExitCode = 1;
    StartupFailureReporter.Write(exception);
}

static bool ShouldReportStartupFailure(Exception exception) =>
    exception is not OperationCanceledException && !IsRunningUnderWebApplicationFactory();

static bool IsRunningUnderWebApplicationFactory() =>
